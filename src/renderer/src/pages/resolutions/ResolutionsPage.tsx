...
const CATEGORIES = ['General Resolutions', 'Commendations', 'Posthumous']

const catColor = (c: string) => {
  if (c === 'General Resolutions') return 'purple'
  if (c === 'Commendations') return 'blue'
  if (c === 'Posthumous') return 'gray'
  return 'gray'
}
...
const columns: Column<Resolution>[] = [
  ...
  { key: 'series', header: 'Series', width: 'w-32' },
  {
    ...
    width: 'w-40',
    render: (r) => (
      <Badge variant={catColor(r.category) as 'purple' | 'blue' | 'gray'}>{r.category}</Badge>
    )
  },
...
export function ResolutionsPage() {
  ...
  const filtered = useMemo(() => {
    const base = items as unknown as Resolution[]
    ...
    return sortByField(result as unknown as Record<string, unknown>[], sortField, sortDirection) as unknown as Resolution[]
    ...
    <Layout>
      <PageContainer>
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                style={{
                  ...
                  transition: 'all 0.2s',
                  background:
                    category === cat ? 'linear-gradient(135deg,#7c3aed,#4f46e5)' : 'transparent',
                  color: category === cat ? '#fff' : 'var(--c-text-3)',
                  boxShadow: category === cat ? '0 2px 8px rgba(124,58,237,0.35)' : 'none'
                }}
              >
                {cat}
              </button>
        ...
        <ConfirmDialog
          ...
          message={`Delete Resolution No. ${selected?.resolutionNumber}?`}
          confirmLabel="Delete"
          danger
          loading={deleting}
