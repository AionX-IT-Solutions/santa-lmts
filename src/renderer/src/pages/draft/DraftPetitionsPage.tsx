...
import { FileText, Plus, RefreshCw, Pencil, Trash2, ExternalLink } from 'lucide-react'
...
import type { DraftPetition } from '../../types'
...
const columns: Column<DraftPetition>[] = [
  { key: 'draftPetitionNumber', header: 'Petition No.', width: 'w-28' },
...
function DraftPetitionFormModal({
  open,
  onClose,
  onSuccess,
  logActivity,
  ...
  record?: DraftPetition
  existingItems: DraftPetition[]
}) {
  const isEdit = !!record
  ...
  useEffect(() => {
    if (open) {
      setForm(
        record
          ? {
              draftPetitionNumber: record.draftPetitionNumber ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.draftPetitionNumber.trim() || !form.title.trim() || !form.sessionNo.trim()) {
      notify.error('Draft number, Session number, and Title are required')
      return
    }
    if (!isEdit && existingItems.some((item) => item.draftPetitionNumber === form.draftPetitionNumber)) {
      notify.error(`Petition No. ${form.draftPetitionNumber} already exists`)
      return
    }
    setSaving(true)
    try {
      if (isEdit)
        await updateDocumentWithFile(
          'santa_draft_petition',
          record!.id,
          { ...form },
          'draftPetitions',
          `DraftPetition_${form.draftPetitionNumber}`,
          file,
          record?.fileUrl ?? '',
          record?.fileType ?? ''
        )
      ...
      await logActivity(`${isEdit ? 'Updated' : 'Created'} Draft Petition ${form.draftPetitionNumber}`)
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? 'Edit Petition' : 'Add Petition'}
      size="lg"
      footer={
        <>
    ...
    >
      <form className="grid grid-cols-2 gap-4">
        <FormField label="Petition Number" required>
          <Input type="number" min="1" value={form.draftPetitionNumber} onChange={set('draftPetitionNumber')} placeholder="e.g. 1" />
        </FormField>
...
export function DraftPetitionsPage() {
  ...
  const { items, loading, loadingMore, hasMore, reload, loadMore, sortField, sortDirection } =
    useListData<Record<string, unknown>>({
      endpoint: 'santa_draft_petition',
      sortParam: 'draftPetitionNumber|desc',
      dataKey: 'draftPetition',
      limit: 100,
      searchQuery: debouncedSearch
    })
  const [selected, setSelected] = useState<DraftPetition | null>(null)
  const [showAdd, setShowAdd] = useState(false)
  const [showEdit, setShowEdit] = useState(false)
  const [showDelete, setShowDelete] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const filtered = useMemo(() => {
    const base = items as unknown as DraftPetition[]
    const q = debouncedSearch.toLowerCase()
    const result = !debouncedSearch.trim() ? base : base.filter(
      (r) => r.title?.toLowerCase().includes(q) || r.draftPetitionNumber?.toLowerCase().includes(q)
    )
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await deleteDocumentWithFile('santa_draft_petition', selected.id, 'draftPetitions', `DraftPetition_${selected.draftPetitionNumber}`)
      await logActivity(`Deleted Draft Petition ${selected.draftPetitionNumber}`)
    ...
    <Layout>
      <PageContainer>
        ...
        <div className="flex justify-end mb-4 shrink-0">
          <input type="text" placeholder="Search by petition no., title..." value={search} onChange={(e) => setSearch(e.target.value)} className="input-field w-56! py-2!" />
        </div>
        <div className="card flex flex-col flex-1 min-h-0">
          <DataTable columns={columns} data={filtered} hiddenColumns={hiddenColumns} selectedId={selected?.id} onRowClick={setSelected} onRowDoubleClick={() => selected?.fileUrl && window.open(selected.fileUrl, '_blank')} loading={loading} emptyMessage="No draft petition records found" loadingMore={loadingMore} onEndReached={hasMore ? loadMore : undefined} />
        </div>
        <DraftPetitionFormModal open={showAdd} onClose={() => setShowAdd(false)} onSuccess={() => { setShowAdd(false); reload() }} logActivity={logActivity} existingItems={filtered} />
        {selected && <DraftPetitionFormModal open={showEdit} onClose={() => setShowEdit(false)} onSuccess={() => { setShowEdit(false); setSelected(null); reload() }} logActivity={logActivity} existingItems={filtered} record={selected} />}
        <ConfirmDialog open={showDelete} title="Delete Draft Petition" message={`Delete Draft Petition ${selected?.draftPetitionNumber}?`} confirmLabel="Delete" danger loading={deleting} onConfirm={handleDelete} onCancel={() => setShowDelete(false)} />
      </PageContainer>
    </Layout>
  )
}
